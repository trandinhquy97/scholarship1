...
<div class="container">
    <div class="leftside">
        <div class="head-post">
            <img class="cover-post" src="{{URL::asset($contestDetail->AnhBia)}}">
            <p id="tit">{{$contestDetail->TenSuKien}}</p>
            <img id="flag-contry" src="{{URL::asset($contestDetail->AnhQuocKy)}}">
            <div class="deadline">Thời gian bắt đầu sự kiện: <span>{{date('d/m/Y', strtotime($contestDetail->ThoiGianBatDauSuKien))}}</span></div>
        </div>
        <p id="author-post">Đăng bởi {{$contestDetail->username}}, {{$contestDetail->created_at}}</p>
        {{--<table>--}}
        {{--<tr>--}}
        {{--<td>--}}
        ...
        <div class="content-info">
            {{$contestDetail->NoiDung}}
        </div>
        <div class="source-link">
            <b>Nguồn: </b>
            <a href="{{$contestDetail->LinkDangKy}}">{{$contestDetail->LinkDangKy}}</a>
    ...
    <div class="rightside">
        ...
        @foreach($contests as $contest)
            <a href="{{'../contest/'.$contest->id_SuKien}}" class="feed-post">
                <div class="item-feed">
                    <img style="position: absolute;width: 40px;height: 40px;top:3px;left: 5px;" src="{{URL::asset($contest->AnhQuocKy)}}">
                    <img class="img-feed" src="{{URL::asset($contest->AnhBia)}}">
                    <div class="bg-tit">
                        <span>{{$contest->TenSuKien}}<br></span>
                    </div>
                </div>
            </a>
        @endforeach
