        ...
        <table id="sorting-table" class="table mb-0">
            ...
            @foreach($posts as $value)
                @php
                    $direct = ''
                @endphp
                @if($value->id_LoaiSuKien==1)
                    @php
                        $direct = 'workshop'
                    @endphp
                @endif
                @if($value->id_LoaiSuKien==2)
                    @php
                        $direct = 'contest'
                    @endphp
                ...
                @endif
                <tr>
                    <td><span class="text-primary">{{$value->id_SuKien}}</span></td>
                    <td><a target="_blank" href="/{{$direct}}/{{$value->id_SuKien}}"> {{str_limit($value->TenSuKien, 32)}}</a></td>
                    <td>{{$value->TenLoaiSuKien}}</td>
                    <td>{{$value->NgayTao}}</td>
                    @switch($value->id_TrangThaiTopic)
                        @case(1)
                        @php ($status = 'success')
                        @break
                        @case(2)
